「짜고 치는 레이스」(가제, 영문 후보 Dirty Derby — §8) — Claude Code 인수인계 문서 (v11)
...
---
7. ★ 다음 작업 큐 (우선순위순)
★ **0순위: 씬 저장 확인** — v11도 씬을 저장하지 않고 끝났다(작업 규칙: 저장은 유저가 Ctrl+S). **조명 A안**(`ambientIntensity` 0.75 / `BettingRoom.lightClosed` 0.30 / `lightOpen` 1.50 / 실내등 0.30)이 미저장일 수 있으니 첫 작업으로 확인할 것. ⚠ 한글 오브젝트를 grep으로 찾으면 안 나온다 — 씬 YAML이 `\uXXXX`로 저장한다(§11). 프리팹·스크립트·폰트·구운 모델은 이미 저장됨.
