「짜고 치는 레이스」(가제, 영문 후보 Dirty Derby — §8) — Claude Code 인수인계 문서 (v7)
...
> **v7 세션(이번): ① 1인칭 껍데기 픽스(PlayerHeadAim 내 머리 회전 제거) ② 쓰러짐 카메라 리얼 연출(머리 본 추종 + 얼굴 앞 눈 앵커) ③ 기상 애니 교체(Anims3, 1.8초) ④ 인스펙터 피격 재생 버튼 ⑤ 커마 패널 UI 폴리싱 ⑥ 타이틀씬 대공사(터레인+원형 트랙+동물 퍼레이드+부스트 쇼+경주 시설물+픽셀 UI 통일+앵커) — §6 참조. 빠따 duel MPPM은 유저 실기 통과.**
---
...
---
4. 파일별 상세 (64개 = v6의 62 + v7 신규 2: TitleTrackShow, PlayerKnockdownEditor. v7 수정: PlayerHeadAim=내 머리 회전 안 함, FirstPersonController=LieEyeBlend 얼굴 앞 눈 앵커, PlayerKnockdown=쓰러짐 머리 추종 카메라+getUpSpeed 1.5, CustomizationPanel=UI 재조립)
Core — 게임 흐름
...
---
— 끝. 코드가 진실, 이 문서는 지도다. 첫 안건은 §7 순서대로: ~~기상 카메라 몸뚫림~~(v7 종결 — 수용된 한계) → ~~빠따 duel MPPM 실기~~(v7 유저 실기 통과) → **상점 미결정 4개 확정/구현(+내구도)** → 감속 아이템 연출 확정/구현 → 멀티 잔여 테스트(복귀·호스트 이탈) → 새 맵 밸런스 검증(+EV 공식 갱신) → V7 입력 확인 → 치킨 40% → 커마 후속 → 킥 ㄱ/아니오 → 성능 최적화.