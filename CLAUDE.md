「짜고 치는 레이스」(가제, 영문 후보 Dirty Derby — §8) — Claude Code 인수인계 문서 (v17)
...
---
7. ★ 다음 작업 큐 (우선순위순)
★ **v17 0순위: GameScene 저장 (Ctrl+S)** — HUD 상단 3패널 + LocalizedLabel 27곳 + BetText 비활성 + 태양 위치·SkyFollowCamera·BeautifySettings.sun 배선 + 피규어테이블 4방 + 회색 선반 비활성이 **전부 미저장**. TitleScene(LocalizedLabel 36곳)은 세션 중 저장 완료. 에셋(SO·머티리얼·폰트·Beautify 프로파일·strings.csv)도 저장 완료.
...
무전기 LCD = **전 언어 영문 대문자 고정** (`AnimalDefinition.LcdName` — LAB디지털 폰트 가나 미지원 + 계기판 감성, 유저 승인. 타이핑 분할은 길이 기반이라 자동 대응).
