「짜고 치는 레이스」(가제, 영문 후보 Dirty Derby — §8) — Claude Code 인수인계 문서 (v20)
...
⚠ **v20 예외 (유저 결정)**: 신규 6종(소/비둘기/낙타/원숭이/북극곰/얼룩말)은 **일단 스킬 None 깡통으로 출전** — `IsActive(None)=false`라 발동 무전기는 패시브처럼 자동 거부, 처형/주사기/펭귄과는 상호작용 없음(효과는 그냥 받음). 스킬을 붙일 때 위 3종 세트 규칙으로 복귀할 것. 풀은 14종이 되어 **매판 랜덤 9종 출전**(8종 전원 출전제 종료).
...
---
7. ★ 다음 작업 큐 (우선순위순)
★ **v20 0순위: GameScene 저장 (Ctrl+S)** — ① RaceManager.animalPool 14종 배선 ② HUD>TimelineFeed 아이콘 3개(↑↓⚡) 배선이 dirty로 대기 중. 저장 안 하면 신규 동물 6종이 풀에서 빠지고 피드 화살표가 안 뜬다.
