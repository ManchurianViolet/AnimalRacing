「짜고 치는 레이스」(가제, 영문 후보 Dirty Derby — §8) — Claude Code 인수인계 문서 (v18)
...
---
7. ★ 다음 작업 큐 (우선순위순)
★ **v18 0순위: GameScene 저장 확인 (Ctrl+S)** — garage 4채·베팅방 이전·콜라이더·천장 1.25배·animalPool 인간 등록·Global Volume 컴포넌트가 씬에 걸려 있다. 세션 중 여러 번 안내했으나 유저 저장 여부 미확인 (v17 미저장분은 세션 초에 저장돼 있었음 — dirty=False 실측).
...
`Assets/Localization/strings.csv` = **유일한 번역 편집 지점** (열: key, ko, en, ja, 비고 / UTF-8 **BOM** / 따옴표·필드 내 줄바꿈 지원). 현재 148키.
